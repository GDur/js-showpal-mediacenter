...
export default class ShowPaper extends React.Component {
  ...
  render() {
    ...
    let episode = series.episodes.map((episode) => {
      ...
      return (<div key={episode.fullFilePath}>
        <FlatButton tabIndex={(
            ...
            // self.state.player.resumeEpisode(series, episode)
            fetch('/extractVideoInformationRequest?videoPath=' + episode.fullFilePath, {accept: 'application/json'}).then(checkStatus).then(parseJSON).then((data) => {

              console.log(this, episode.fullFilePath, JSON.stringify(data))
            });

          }} icon={<AVPlayArrowIcon/>} style={style} label={episode.getBeautifiedName()}/>

        <IconButton tabIndex={1} className="expand-button" onClick={() => {
            let path = '/conversionRequest?videoPath=' + episode.fullFilePath

            return fetch(path, {accept: 'application/json'}).then(checkStatus).then(parseJSON).then((data) => {
              console.log(data)
            });
          }} tooltip="convert episode" touch={true} tooltipPosition="bottom-right">
          <MovieFilterIcon/>
        </IconButton>
        <IconButton tabIndex={1} className="expand-button" onClick={() => {
            let path = '/showInExplorerRequest?videoPath=' + episode.fullFilePath

            return fetch(path, {accept: 'application/json'}).then(checkStatus).then(parseJSON).then((data) => {
              console.log(data)
            });
          }} tooltip="show file in explorer" touch={true} tooltipPosition="bottom-right">
          <FileFolder/>
        </IconButton>
    ...
    let episodeView = null;

    if (this.state.showDetails) {
      episodeView = <div className={'details show-details'}>{episode}
      </div>
    } else {

      episodeView = <div className={'details hide-details'}></div>
    }

    return (<Paper className="series-container" zDepth={2}>
      {/* <img src={image}/> */}
      <IconButton tabIndex={1} className="expand-button" onClick={this.onExpandChange} tooltip="show episodes" touch={true} tooltipPosition="top-right">
        {
          this.state.showDetails
      ...
      <FlatButton tabIndex={1} className="play-series-button" onClick={() => {
        ...
        }} icon={<AVPlayArrowIcon/>} style={style} label={series.name}/>
      {episodeView}
    </Paper>)
  }
}
